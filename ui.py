    ...
    if not st.session_state.get("logged_in", False):
        ...
        with st.sidebar:
            ...
            if st.session_state.welcome:
                st.toast(f"Добро пожаловать, {st.session_state.username}!", icon="✅")
                st.session_state.welcome = False


def current_words_init():
    if "current_words" not in st.session_state or st.session_state.current_words == None:
        st.session_state.current_words = services.generate_options(st.session_state.user_id)
        st.session_state.current_word = random.choice(st.session_state.current_words)
...
def render_study_tab():
    ...
    if st.session_state.is_right:
        st.button(
            "➡️Следующее слово", on_click=new_words_init, use_container_width=True
        )
...
def render_add_word_tab():
    ...
    if st.button("Добавить слово"):
        if services.add_personal_word(st.session_state.user_id, rus_w, eng_w) == True:
